...
asyncTest "when the promise is resolved, all of the observers are notified", ->
  expect 2
  callback1 = (value) ->
    equal value, "hello"
  ...
  wp.then(callback1)
  wp.then(callback2)
  # This failing test proves that callback1 isnt getting called, and furthermore
    # that we don't have the right code to handle more than one callback. It proves
    # that only the *last* caller of the then method determines the callback to be
    # used